...
class IoTraspicover(IoTDeviceBase):
    ...
    # translate above constants into the proper values as cover config says and as HA/MQTT understand
    translate_status = {}
    ...
    def __callback(self, pin, cfg):
        ...
        if state_pin_up and state_pin_down:
            self.logger.error("IoTraspicover.__callback(): both input pins true --> illegal state" )
        ...
        if pin == pin_up:
            ...
            self.logger.error("IoTraspicover.__callback(): unknown pin %d, something is wrong", pin )
    ...
    def __init__(self, **kwargs):
        ...
        for cover in covers_cfg:
            ...
            GPIO.setup(pin_up, GPIO.IN, pull_up_down=GPIO.PUD_UP)  # GPIO Modus zuweisen
            GPIO.setup(pin_down, GPIO.IN, pull_up_down=GPIO.PUD_UP)  # GPIO Modus zuweisen
            GPIO.setup(pin_trigger, GPIO.OUT)  # GPIO Modus zuweisen

            state_pin_up= not GPIO.input(pin_up)
            state_pin_down= not GPIO.input(pin_down)

            if not state_pin_up and not state_pin_down:
                cfg["status"]= IoTraspicover.STATE_UNKNOWN #self.conf["state_unknown"]
            elif state_pin_up and not state_pin_down:
                cfg["status"]= IoTraspicover.STATE_OPEN #self.conf["state_open"]
            elif not state_pin_up and state_pin_down:
                cfg["status"]= IoTraspicover.STATE_CLOSED #self.conf["state_closed"]
            else:
                self.logger.error("IoTraspicover.__init()__: both input pins true --> illegal state" )
                cfg["status"]= IoTraspicover.STATE_ILLEGAL

            self.logger.info("new IoTraspicover device with input pins %d, %d and output pin %d, initial state %s ", 
                             pin_down, pin_up, pin_trigger, self.translate_status[cfg["status"]] )

            self.covers[cover] = cfg

            GPIO.add_event_detect( pin_up, GPIO.BOTH, callback= callback, bouncetime=100 )
            GPIO.add_event_detect( pin_down, GPIO.BOTH, callback= callback, bouncetime=100 )
            ...
            if cover in messages:
                ...
                if msg == self.conf["payload_open"]:
                    ...
                    if IoTraspicover.STATE_OPEN == oldstate:
                        ...
                        self.logger.error("IoTraspicover.set_state(): unknown previous state %s", oldstate )
                    ...
                    self.logger.error("IoTraspicover.set_state(): unknown command %s", msg )

        # return False so that the mqtthass will _not_ publish the updated state right away
        return False 
