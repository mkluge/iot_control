...
class IoTRuntime:
        ...
        for backend in self.conf["backends"]:
            ...
            try:
                self.backends.append(
                    IoTFactory.create_backend(backend, config=backend_cfg)
                )
            except Exception as exception:
                self.logger.error("error creating backend: %s", exception)
    ...
    def connect_devices(self):
        for device in self.conf["devices"]:
            ...
            try:
                real_device = IoTFactory.create_device(device, config=device_cfg)
                real_device.give_runtime_reference(self)
                self.devices.append(real_device)
                for backend in self.backends:
                    backend.register_device(real_device)
            except Exception as exception:
                self.logger.error("error creating device: %s", exception)
        for backend in self.backends:
            backend.announce()
            ...
            try:
                ...
                if data:
                    try:
                        ...
                        self.logger.error("error writing to backend %s".format(backend))
                        self.logger.error(error)
                else:
                    self.logger.error(
                        "unable to read data from device %s".format(device)
                    )
            except Exception as error:
                self.logger.error("error reading device ".format(device))
                self.logger.error(error)
    ...
    def schedule_for_device(self, delay, device, switch, event):
        ...
        self.loop.call_soon_threadsafe(
            functools.partial(
                IoTRuntime.__schedule_from_local_thread,
                self,
                delay,
                device,
                switch,
    ...
    def trigger_for_device(self, device):
        ...
        self.loop.call_soon_threadsafe(
            functools.partial(IoTRuntime.triggered_update, self, device)
        )
